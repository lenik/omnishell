...
void ControlPanelBody::createFragmentView(CreateViewContext* ctx) {
    ...
    m_categoryList->Bind(wxEVT_LIST_ITEM_SELECTED, &ControlPanelBody::OnCategorySelected, this);
    m_categoryList->Bind(wxEVT_LIST_ITEM_ACTIVATED, &ControlPanelBody::OnModuleDoubleClicked, this);
...
void ControlPanelBody::showModuleManager() {
    ...
    enableBtn->Bind(wxEVT_BUTTON, [moduleList, modules, refreshStatus](wxCommandEvent&) {
        long item = -1;
    ...
    disableBtn->Bind(wxEVT_BUTTON, [moduleList, modules, refreshStatus](wxCommandEvent&) {
        long item = -1;
        item = moduleList->GetNextItem(item, wxLIST_NEXT_ALL, wxLIST_STATE_SELECTED);
        if (item == -1 || item >= (long)modules.size())
            return;
