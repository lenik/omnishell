...
DirIndex::DirIndex(VolumeManager* volumeManager)
...
std::string DirIndex::handleGet(size_t volumeIndex, const std::string& pathInfo) {
    ...
    return generateHtml(volumeIndex, volumePath, "/volume/" + std::to_string(volumeIndex) + "/" + pathInfo);
}

std::string DirIndex::generateHtml(size_t volumeIndex, const std::string& volumePath, const std::string& urlPath) {
    auto volume = m_volumeManager->getVolume(volumeIndex);
    ...
    html << "<!DOCTYPE html>\n"
         ...
         << "<th class=\"name diridx-th\" data-sort-key=\"name\" tabindex=\"0\" scope=\"col\">Name<span class=\"sort-ind\" aria-hidden=\"true\"></span></th>"
         << "<th class=\"date diridx-th\" data-sort-key=\"mtime\" tabindex=\"0\" scope=\"col\">Last modified<span class=\"sort-ind\" aria-hidden=\"true\"></span></th>"
         << "<th class=\"size diridx-th\" data-sort-key=\"size\" tabindex=\"0\" scope=\"col\">Size<span class=\"sort-ind\" aria-hidden=\"true\"></span></th>"
         << "<th class=\"date diridx-th\" data-sort-key=\"desc\" tabindex=\"0\" scope=\"col\">Description<span class=\"sort-ind\" aria-hidden=\"true\"></span></th>"
         << "</tr></thead>\n"
    ...
    if (volumePath != "/") {
        ...
        if (lastSlash != std::string::npos) {
            ...
            if (parentPath.empty()) parentPath = "/volume/" + std::to_string(volumeIndex) + "/";
        }
    ...
    for (const auto& entry : entries) {
        ...
        if (!entry->isDirectory()) {
            description = FileTypeDetector::getFileDescription(entry->name);
        }

        const std::string sortName = toLowerAscii(entry->name + (entry->isDirectory() ? "/" : ""));
        const long long sortMtime = entry->modifiedTime > 0 ? static_cast<long long>(entry->modifiedTime) : 0LL;
        const long long sortSize = entry->isDirectory() ? 0LL : static_cast<long long>(entry->size);
        const std::string sortDesc = toLowerAscii(description);
        
        html << "<tr data-sort-name=\"" << htmlAttrEscape(sortName) << "\" data-sort-mtime=\"" << sortMtime
             << "\" data-sort-size=\"" << sortSize << "\" data-sort-desc=\"" << htmlAttrEscape(sortDesc) << "\">"
    ...
    html << "</tbody>\n"
         ...
         << "    if (k === 'mtime') {\n"
         << "      var na = parseInt(a.dataset.sortMtime, 10) || 0, nb = parseInt(b.dataset.sortMtime, 10) || 0;\n"
         << "      return d * (na - nb);\n"
         << "    }\n"
         << "    if (k === 'size') {\n"
         << "      var na = parseInt(a.dataset.sortSize, 10) || 0, nb = parseInt(b.dataset.sortSize, 10) || 0;\n"
         << "      return d * (na - nb);\n"
         << "    }\n"
         ...
         << "    var rows = Array.prototype.slice.call(tbody.querySelectorAll('tr:not(.diridx-parent)'));\n"
         << "    rows.sort(cmp);\n"
         ...
         << "    document.querySelectorAll('th.diridx-th .sort-ind').forEach(function(s) { s.textContent = ''; });\n"
         << "    var th = document.querySelector('th.diridx-th[data-sort-key=\"' + state.key + '\"]');\n"
         << "    if (th) { var ind = th.querySelector('.sort-ind'); if (ind) ind.textContent = state.dir > 0 ? '\\u25b2' : '\\u25bc'; }\n"
         << "  }\n"
