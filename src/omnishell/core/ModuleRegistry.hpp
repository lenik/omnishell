...
class ModuleRegistry {
  public:
    using ModuleFactory = std::function<ModulePtr(CreateModuleContext*)>;
    ...
    /**
     * Register a module factory globally.
     *
     * This should typically be called only from static initializers via
     * the OMNISHELL_REGISTER_MODULE macro. The factories are copied into
     * each ModuleRegistry instance on construction.
     */
    static void registerModuleFactory(const std::string& uri, ModuleFactory factory);
...
#define OMNISHELL_REGISTER_MODULE(uri, module_class)                                               \
    ...
    struct module_class##_Registrar {                                                              \
        module_class##_Registrar() {                                                               \
            os::ModuleRegistry::registerModuleFactory(                                             \
                uri, [](os::CreateModuleContext* ctx) -> os::ModulePtr {                           \
                    return std::make_shared<module_class>(ctx);                                    \
                });                                                                                \
        }                                                                                          \
    };                                                                                             \
